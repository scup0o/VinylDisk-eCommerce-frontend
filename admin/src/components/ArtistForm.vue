        ...
        <div class="modal-wrapper">
            <div class="modal-container">
                <div class="modal-header">
                    <label class="" style="margin: auto; font-size: 20px;" v-if="edit===false">Thêm nghệ sĩ</label>
                    <label class="" style="margin: auto; font-size: 20px;" v-else>Chỉnh sửa nghệ sĩ</label>

                </div>
                    <Form @submit="createArtist" :validation-schema="FormSchema">
                        <div class="modal-body">
                        <div class="row">
                            <div class="form-group">
                                <label for="name">Tên nghệ sĩ: </label>
                                <Field name="name" type="text" class="form-control" v-model="artistNew.name">
                                </Field>
                        ...
                        <div style="padding-top:20px">
                            ...
                            <form encType="multipart/form-data" @submit="this.$emit('submit')" action="/api/img" method="post">
                                ...
                                <input type=file name='fileuploaded' style="visibility: hidden;" @change="onFileChange" encType="multipart/form-data" id="fileuploaded">
                                <div class="row">
                                <div v-if="artistNew.image && haveData===true">
                                    <img style="display: inline-flex;" class="preview img-thumbnail" :src="`../../src/assets/img/${artistNew.image}`"/>
                                    {{ artistNew.image }}
                                </div>
                                <div v-for="(image, key) in images">
                                    <div :id="key">
                                        <img style="display: inline-flex;" class="preview img-thumbnail" v-bind:ref="'image' +parseInt( 0 )" /> 
                                        {{ image.name }}
                                    </div>
                                </div>
                                </div>
    ...
    methods: {
        ...
        async createArtist(data) {
            try {
                ...
                if (this.images[0] !== undefined) {data.image = this.images[0].name;}
                if (this.edit === false){
                    const check = await ArtistService.create(data);
                    if (check === false){
                        this.nameMessage = 'Tên nghệ sĩ đã tồn tại'
                    }else{
                    ...
                    const check = await ArtistService.update(this.artistNew._id, data);
                    if (check === false){
                        this.nameMessage = 'Tên nghệ sĩ đã tồn tại'
                    }
                    else{
                        const headers = { 'Content-Type': 'multipart/form-data' };
                            if (this.images[0] !== undefined) await ImgService.upload(this.img, { headers });
                            this.$toast.open({
                                message: "Chỉnh sửa nghệ sĩ thành công",
                                type: "success",
                                duration: 3000 ,
                                dismissible: true
                            })
